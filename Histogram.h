...
enum KierunekLiczenia
{
  OdPoczTablicy, OdKoncaTablicy
};
...
public:
  ...
  Histogram();
  Histogram(
      TYP* wskNowejTablicyDanych,	//wersja dynamicznie modyfikujaca poczatek i koniec
      long int NowaIloscDanych = 1, int IloscPrzedzialow = 0,
      KierunekLiczenia NowyKierunek = OdPoczTablicy);

  Histogram(
      TYP* wskNowejTablicyDanych,	//wersja o stalych przedzialach
      TYP poczatek, TYP koniec, long int NowaIloscDanych = 1,
      int IloscPrzedzialow = 0, KierunekLiczenia NowyKierunek = OdPoczTablicy);
  ...
  void DodajDaneInnaTablica(long int IloscNowychDanych,
      TYP* wskInnaTablicaDanych);
...
ostream& operator<<(ostream & stru, Histogram<TYP> & x)
{
  ...
  stru << "Kierunek Liczenia od poczatku(0)/konca(1): "
      << x.KierunekLiczeniaDanych << endl << endl;
  stru << "ILOSCI ZLICZEN W PRZEDZIALACH:" << endl;
  for (int i = 0; i < x.IloscPrzedz; i++)
    stru << x.wskTablicyPrzedzialow[i] << endl;
  stru << endl;
}
#endif
//-----------------------------------------------------------------------------------------------------------
