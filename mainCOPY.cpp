...
static int A, B, ile = 2, par = 6, nrmojebin = 0, nrmojetxt = 0, nrbin = 0,
    nrtxt = 0, IleRazySONO = 2500, IleRazyBONO = 300, CoIleKrokowZapis = 10;

static long int LicznikKrokow = 0, LicznikSONO = 0, LicznikBONO = 0;

Wektor3D* wskTablicyPomocniczejWektorow;
Wektor3D* wskPomocniczyWektora1;
Wektor3D* wskPomocniczyWektora2;

static double x = 0, y = 0, Epsilon = 1E-12, StosunekPomocniczy = 1,
    Energia = 0, EnergiaNowa = 0, EnergiaPocz = 0, WagaEnergiiOdpych = 0,
    StosunekGestosciStanow = 0.5, DlugoscWezlaPoczatkowa, DlugoscWezla,
    PromienMaksObiektu;
...
static void display(void)
{
    ...
    for (int i = 0; i < IleRazyBONO; i++)          //PETLA OBLICZEN
    {
      ...
      else                            ///uwarunkowania energetyczne
      {
        ...
        {       //jesli nowy stan ma wyzsza energie
          ...
          StosunekGestosciStanow = powl(M_E,
              (Energia - EnergiaNowa) / (0.0001 * EnergiaPocz));
          ...
          { //jesli nie zmiescilismy sie w tej liczbie (mniejszej niz 1) //przywracamy stan poprzedni:
            ...
            { //niedzielona
              wskPomocniczyWektora1 =
                  &(wskTablicyPomocniczejWektorow[W.PunktGiecia1 + 1]);
              wskPomocniczyWektora2 = &(W.wskTablicyElem[W.PunktGiecia1 + 1]);
              for (int i = W.PunktGiecia1 + 1; i < W.PunktGiecia2; i++)
              {
                *wskPomocniczyWektora2++ = *wskPomocniczyWektora1++;
  ...
  {    //KOMUNIKATY KONSOLOWE
    cout << "Dlugosc wezla: \t\t" << DlugoscWezla << endl << "StosunekGS: \t\t"
        << StosunekGestosciStanow << endl;
    cout << "Energia: \t\t\t" << Energia << endl << "Energia Nowa: \t\t\t"
        << EnergiaNowa << endl;
    cout << "Energia Pocz: \t\t\t" << EnergiaPocz << endl
        << "Licznik SONO: \t\t\t" << LicznikSONO << endl;
    cout << "Promien: \t\t\t" << W.PromienPrzekroju << endl
        << "Licznik BONO: \t\t\t" << LicznikBONO << endl;
  }
