...
static long int LicznikKrokow=0,
                LicznikSONO=0,
                LicznikBONO=0;
...
static double x=0,y=0,
              Epsilon=1E-12,
              StosunekPomocniczy=1,
              Energia=0,
              EnergiaNowa=0,
              EnergiaPocz=0,
              WagaEnergiiOdpych=0,
              StosunekGestosciStanow=0.5,
...
display(void)
{
 ...
 if(SONO) //algorytm zaciskania
   {
    if(!(LicznikKrokow%CoIleKrokowZapis++))  //zapisywanie do pliku
      {
 ...
 if(BONO)  //ALGORYTM ROZPLATUJACY
   {
    ...
    for(int i=0;i<IleRazyBONO;i++)          //PETLA OBLICZEN
       {
        ...
        {//zapis stanu wyginanej czesci przed gieciem:
	...
	if(W.PunktGiecia1<W.PunktGiecia2) //okreslenie kierunku przedzialu dla potrzeb
                                           //sprawdzania przekryc
        ...
        if(W.SprawdzCzySaPrzekrycia(A,B) ///uwarunkowania podstawowe (krzywizna i twardosc)
         ...
         else                            ///uwarunkowania energetyczne
           {
            {//OBLICZENIE NOWEJ ENERGII WEZLA
             EnergiaNowa=/*W.WyznaczEnergieKrzyw()*/ 
                         +WagaEnergiiOdpych*W.WyznaczEnergieOdpych();
            }
            if(Energia<EnergiaNowa)
              {       //jesli nowy stan ma wyzsza energie 
                 //obliczamy Stosunek Gestosci Stanow dla tych energii
                 StosunekGestosciStanow=powl(M_E,(Energia-EnergiaNowa)/(0.0001*EnergiaPocz));
 ...
 {//ENERGIa WEZLA ///+W.WyznaczEnergieKrzyw()
  Energia=WagaEnergiiOdpych*W.WyznaczEnergieOdpych();
 }
 ...
 {//KOMUNIKATY KONSOLOWE
  cout<<"Dlugosc wezla: \t\t"<<DlugoscWezla<<endl<<"StosunekGS: \t\t"<<StosunekGestosciStanow<<endl;
  cout<<"Energia: \t\t\t"<<Energia<<endl<<"Energia Nowa: \t\t\t"<<EnergiaNowa<<endl;
  cout<<"Energia Pocz: \t\t\t"<<EnergiaPocz<<endl<<"Licznik SONO: \t\t\t"<<LicznikSONO<<endl;
  cout<<"Promien: \t\t\t"<<W.PromienPrzekroju<<endl<<"Licznik BONO: \t\t\t"<<LicznikBONO<<endl;
 }
 ...
 for(int i=0; i<W.IloscElem; i++)              //RYSOWANIE
    ...
    if(W.ElementyPrzekrKrzyw.find(i)!=W.ElementyPrzekrKrzyw.end()) {rr+=0.63; gg=0;}
...
Wybierz(int button, int state, int a, int b)
{
 ...
 for(int i=0;!koniec;i++)
    ...
    if((W.wskTablicyElem[n]-P).length()<W.PromienPrzekroju||i>100) {koniec=1; wybr=n;};
    }; 
...
main(int argc, char *argv[])
{
    ...
    Pomoc=W;
    WagaEnergiiOdpych=1;//W.WyznaczEnergieKrzyw()/W.WyznaczEnergieOdpych()/2;

    EnergiaPocz=Energia=EnergiaNowa=//W.WyznaczEnergieKrzyw()
           +WagaEnergiiOdpych*W.WyznaczEnergieOdpych();
