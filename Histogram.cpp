...
Histogram<TYP>::Histogram(TYP* wskNowejTablicyDanych, //konstruktor nie dba o poprawnosc danych!!!
    ...
    int IloscPrzedzialow, KierunekLiczenia NowyKierunek) :

    IloscDanych(NowaIloscDanych), KierunekLiczeniaDanych(NowyKierunek),
        StalePrzedzialy(false)
{
  TYP* wskSkoczek;

  if (KierunekLiczeniaDanych = OdPoczTablicy)
  {
    wskPoczatekTablicyDanych = wskNowejTablicyDanych;
  }
  else
  {
    wskKoniecTablicyDanych = wskNowejTablicyDanych;
  };

  if (IloscDanych <= 0 || wskNowejTablicyDanych == NULL)
  {  //jesli za malo danych
    IloscDanych = 0;
    DanaMax = 0;
    DanaMin = 0;
    IloscPrzedz = 1;
  ...
  {  //jesli nie brak danych do kreacji
    if (IloscPrzedzialow > 0)
      IloscPrzedz = IloscPrzedzialow;
    else
      IloscPrzedz = DOMYSLNA_ILOSC_PRZEDZIALOW_HISTOGRAMU;
    ...
    {  //normalne zliczanie danych
      ...
      for (long int i = 0; i < IloscDanych; i++)
        wskTablicyPrzedzialow[static_cast<int>((*(wskSkoczek++) - DanaMin)
            / SzerokoscPrzedz)]++;
    }
    else
    {  //zliczanie danych od konca tablicy
      ...
      for (long int i = IloscDanych; i > 0; i++)
        wskTablicyPrzedzialow[static_cast<int>((*(wskSkoczek++) - DanaMin)
            / SzerokoscPrzedz)]++;
    }
  }
...
template<class TYP>
Histogram<TYP>::Histogram(
    TYP* wskNowejTablicyDanych,	//wersja o stalych przedzialach
    TYP poczatek, TYP koniec, long int NowaIloscDanych, int IloscPrzedzialow,
    KierunekLiczenia NowyKierunek) :

    IloscDanych(NowaIloscDanych), KierunekLiczeniaDanych(NowyKierunek),
        PoczatekPrzedz(poczatek), KoniecPrzedz(koniec), StalePrzedzialy(true)
{
  TYP* wskSkoczek;
  ...
  {	//jesli nie brak danych do kreacji
    ...
    {	//normalne zliczanie danych
      ...
      for (long int i = 0; i < IloscDanych; i++)
      {
        if (((*wskSkoczek) >= PoczatekPrzedz) && ((*wskSkoczek) < KoniecPrzedz))
          if (*wskSkoczek >= PoczatekPrzedz && *wskSkoczek < KoniecPrzedz)
            wskTablicyPrzedzialow[int(
                (*(wskSkoczek) - PoczatekPrzedz) / SzerokoscPrzedz)]++;
        wskSkoczek++;
      }
    ...
    {	//zliczanie danych od konca tablicy
      ...
      for (long int i = IloscDanych; i > 0; i++)
        if (*wskSkoczek >= PoczatekPrzedz && *wskSkoczek < KoniecPrzedz)
          wskTablicyPrzedzialow[static_cast<int>((*(wskSkoczek--)
              - PoczatekPrzedz) / SzerokoscPrzedz)]++;
    };
  };
  IloscDanychPrzedzMax = ZnajdzMax(wskTablicyPrzedzialow, IloscPrzedz);
}
...
void Histogram<TYP>::ZapiszBin(char* Nazwa)   //zapisuje do pliku sekwencyjnego 
{
  ...
  for (int i = 0; i < IloscPrzedz; i++)
    strPliku.write((char*) (&wskTablicyPrzedzialow[i]), sizeof(long int));
  strPliku.close();
}
//--------------------------------------------------------------------------------------------
template<class TYP>
void Histogram<TYP>::WczytajBinStr(ifstream* wskstrPliku)
{
  ...
  wskstrPliku->read((char*) (&IloscDanychPrzedzMax),
      sizeof(IloscDanychPrzedzMax));
  cout << "wczytano IloscDanychPrzedzMax = " << IloscDanychPrzedzMax << endl
      << endl << flush;
  ...
  for (int i = 0; i < IloscPrzedz; i++)
    wskstrPliku->read((char*) (&wskTablicyPrzedzialow[i]), sizeof(IloscPrzedz));
