...
bool KnotSimData::GenerujZTablicyGiec(GiecieWezla* wskGiec,
        ...
        long long int iloscwszystkichgiec, long long int iloscwszystkichudanychgiec,
        int liczbasegmentow)
{
 ...
 wskHistPunktyA=new Histogram<int>(wskTablicaA,0,liczbasegmentow,iloscudanych,liczbasegmentow);
 wskHistOdlAB=new Histogram<int>
                  (wskTablicaOdlAB,2,(liczbasegmentow+1)/2+1,iloscudanych,(liczbasegmentow+1)/2-1);
 ...
 Histogram<double> Probny(wskOdlrazyKat,-6*Odch,6*Odch,iloscudanych,200);
 pointer=Probny.wskTablicyPrzedzialow;
 Probny.IloscDanychPrzedzMax=ZnajdzMax(pointer,Probny.IloscPrzedz);
 int i=0;
 for(;Probny.wskTablicyPrzedzialow[i]!=Probny.IloscDanychPrzedzMax;i++); ///szukamy i startowe dotad OK
 do PlusGranica=Probny.PoczatekPrzedz+Probny.SzerokoscPrzedz*(i+1);
    //cout<<"PlusGranica:   "<<PlusGranica<<endl<<endl<<flush;}
 while(Probny.IloscDanychPrzedzMax/
       (Probny.wskTablicyPrzedzialow[i]?Probny.wskTablicyPrzedzialow[i]:1)<=100&&
        i++<Probny.IloscPrzedz);

 wskHistOdlABrazyKatyGiec=new Histogram<double>(wskOdlrazyKat,-PlusGranica,PlusGranica,iloscudanych,200);
 }

 pointer=wskHistPunktyA->wskTablicyPrzedzialow;
 wskHistPunktyA->IloscDanychPrzedzMax=ZnajdzMax(pointer,wskHistPunktyA->IloscPrzedz);

 pointer=wskHistOdlAB->wskTablicyPrzedzialow;
 wskHistOdlAB->IloscDanychPrzedzMax=ZnajdzMax(pointer,wskHistOdlAB->IloscPrzedz);

 pointer=wskHistKatyGiec->wskTablicyPrzedzialow;
 wskHistKatyGiec->IloscDanychPrzedzMax=ZnajdzMax(pointer,wskHistKatyGiec->IloscPrzedz);

 pointer=wskHistOdlABrazyKatyGiec->wskTablicyPrzedzialow;
 wskHistOdlABrazyKatyGiec->IloscDanychPrzedzMax=ZnajdzMax(pointer,wskHistOdlABrazyKatyGiec->IloscPrzedz);
}
//-----------------------------------------------------------------------------------
void KnotSimData::ZapiszBin(char* nazwapliku)
{
 ...
 strPliku.write((char*)(&IloscWszystkichUdanychGiec),sizeof(IloscWszystkichUdanychGiec));
 strPliku.close();
...
bool  KnotSimData::WczytajBin(char* nazwapliku, long int nr)
{
 ...
 strPliku.read((char*)&IloscWszystkichUdanychGiec,sizeof(IloscWszystkichUdanychGiec));
 ...
 cout<<"wczytano IloscWszystkichUdanychGiec = "<<IloscWszystkichUdanychGiec<<endl<<endl<<flush;
...
{
 stru<<"============= DANE SYMULACJI WEZLA ==================="<<endl<<endl;
 ...
 stru<<"IloscWszystkichUdanychGiec: \t\t\t"<<x.IloscWszystkichUdanychGiec<<endl<<endl;
...
//-------------------------------------------------------------------------------
KnotSimData::KnotSimData()  : IloscGiec(0), 
                                IloscUdanychGiec(0), 
                                IloscWszystkichGiec(0),
...
//---------------------------------------------------------------------------------
KnotSimData::KnotSimData(GiecieWezla* wskGiec,
        long int iloscgiec,long int iloscudanych,
        long long int iloscwszystkichgiec, long long int iloscwszystkichudanychgiec,
        int liczbasegmentow)
{
 GenerujZTablicyGiec(wskGiec,iloscgiec,iloscudanych,
                    iloscwszystkichgiec, iloscwszystkichudanychgiec,
                    liczbasegmentow);
}
//-------------------------------------------------------------------------------
