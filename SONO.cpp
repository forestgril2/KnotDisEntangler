...
// --------------------------------------------------------------------------
CORECT::CORECT() :
    Wezel(NULL), Si(0), TablicaNumerowElementowA(NULL),
        TablicaNumerowElementowB(NULL), TablicaElementow(NULL),
        SredniaDlugosc(1), Sb(1), DlugoscTrwania(4), ParametrZaciskania(0.3),
        Ot(1E-10)
{
}
// --------------------------------------------------------------------------
...
{
  int *NTablicaNumerowElementowA = NULL, *NTablicaNumerowElementowB = NULL;
  int Nowa = RezSt(nad, &NTablicaNumerowElementowA, &NTablicaNumerowElementowB);
  delete[] TablicaNumerowElementowA;
  TablicaNumerowElementowA = NTablicaNumerowElementowA;
  delete[] TablicaNumerowElementowB;
  TablicaNumerowElementowB = NTablicaNumerowElementowB;
  Si = Nowa;
}
//---------------------------------------------------------------------------
...
void CORECT::FStykPR(void)
{
  ...
  {
    i = TablicaNumerowElementowA[k];
    j = TablicaNumerowElementowB[k];
    LH = Wezel[j] - Wezel[i];
    lh = modul(LH);
    if (lh < Om)
    {
...
void CORECT::Zaciskaj(void)
{
  ...
  for (int i = 0; i < IloscSegm; i++)
    TablicaElementow[i] = ZERO;
  FStykPR();
  FSegmentPR();
  for (int i = 0; i < IloscSegm; i++)
    TablicaElementow[i] *= oDlugoscTrwania;
  for (int i = 0; i < IloscSegm; i++)
    Wezel[i] += TablicaElementow[i];
  Sprawdz();
}
//---------------------------------------------------------------------------
int CORECT::RezSt(double Nd, int **ta, int **tb)
{
  ...
  double SredniaDlugosc_1 = IloscSegm / DlugoscWezla, J1S = 1 + SredniaDlugosc,
      J2S = J1S + SredniaDlugosc, odl = 1.0 + Nd, lh;

  int mg = static_cast<int>(
      SredniaDlugosc <= 1.0 ? 0.5 * M_PI / asin(SredniaDlugosc) - 1e-10 : 1),
      StIn = 0;
