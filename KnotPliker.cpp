...
int KnotPliker::WczytajPlik(char* nazwa)
{
 ...
 int	  Segm;
 int	  Giecia;
 cout<<"Wczytywanie pozycji wezlow w pliku"<<" "<<NazwaPliku<<"...."<<ProcentWczytany<<"%\r";
 cout<<flush;
 ...
 cout<<"Wczytywanie pozycji wezlow w pliku "<<NazwaPliku<<"....100%  Zrobione!"<<endl<<endl;
}
//-------------------------------------------------------------------------------------------
void KnotPliker::WczytajKnot(Knot3D* wskWezla, long int Numer)
{
 if(Numer<0) {Numer=AktualnyKnot=0; WczytajKnot(wskWezla,AktualnyKnot);};
 if(Numer>=IloscWezlow) {
 ...
 else
   {
    ...
    for(int i=0;i<wskWezla->IloscGiec;i++)
       {
        wskstrPW->read((char*)(&wskWezla->OstatnieGiecia[i]), sizeof(GiecieWezla));
       }
...
//---------------------------------------------------------------------------------------------
void KnotPliker::ZapiszCoIle(char* wskNazwy,int CoIle,long int Start,long int Koniec)
{
