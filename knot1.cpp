...
double x = 0, y = 0, Energia = 0, EnergiaNowa = 0, EnergiaPocz = 0,
    WagaEnergiiOdpych = 0;
...
static void obliczenia(void)
{
  ...
  if (n)
  {
    for (int i = 0; i < ile; i++)
    {
      ...
      double KatGiecia = W.Losownik.LosujLongDouble(-W.KatGieciaMax,
          W.KatGieciaMax);
      ...
      EnergiaNowa = W.WyznaczEnergieKrzyw()
          + 0.7 * WagaEnergiiOdpych * W.WyznaczEnergieOdpych();
      ...
      else
      {
        ...
        {
          double StosunekGestosciStanow = pow(M_E,
              (Energia - EnergiaNowa) / (0.01 * EnergiaPocz));
...
int main(int argc, char *argv[])
{
  ...
  EnergiaPocz = Energia = W.WyznaczEnergieKrzyw()
