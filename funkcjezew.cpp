...
//------------------------------------------------------------------------------
Wektor3D WyznaczSrodekMasy(const Wektor3D* wskTablicy1, int Ile,
                           int Punkt1, int Punkt2)
                           ...
                           //-----zszablonowac------------
{
 ...
 if (Ile<=0||Punkt1<0||Punkt2<0||Punkt1>Ile||Punkt2>Ile) return Suma/0.0;
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
Macierz3D WyznaczMacierzBezwl(const Wektor3D* wskTablicy1, int Ile,
                              int Punkt1, int Punkt2)
{
 ...
 if (Ile<0||Punkt1<0||Punkt2<0||Punkt1>Ile||Punkt2>Ile) return MacierzBezwl/0.0;
     ...
     for(int p=0; p<Ile; p++) //dla kazdego punktu
        ...
        for (int i=0; i<3; i++)
                ...
                for (int j=i; j<3; j++)
                        ...
                        MacierzBezwl.at(i,j)+=
                               (x*x+y*y+z*z)*delta(i,j)-xyz[i]*xyz[j];
                        };
                };
...
{
 Wektor3D A=wskSegmenty[0]-wskSegmenty[-1],
          B=wskSegmenty[1]-wskSegmenty[0],
          Wektor=A.crossProduct(B); //chwilowo iloczyn wektorowy
 ...
 //zadziwiajaco krotkie, jak to uzyskalem?.. :/ (to jest dobre! :D)
  ...
  f=(bx*(ax+bx)+by*(ay+by)+bz*(az+bz))/
    (az*az*(bx*bx+by*by)+ay*ay*(bx*bx+bz*bz)+ax*ax*(by*by+bz*bz)
    -2*ay*by*(ax*bx+az*bz)-2*ax*az*bx*bz);

 Wektor.x=wskSegmenty[-1].x+0.5*(ax+f*((ay*ay+az*az)*bx-(ay*by+az*bz)*ax));
 Wektor.y=wskSegmenty[-1].y+0.5*(ay+f*((ax*ax+az*az)*by-(ax*bx+az*bz)*ay));
 Wektor.z=wskSegmenty[-1].z+0.5*(az+f*((ax*ax+ay*ay)*bz-(ax*bx+ay*by)*az));
                                   ...
                                   //nie uwzglednia ostatniego
{
 ...
 if (Ile<=0||Segment1<0||Segment2<0||Segment1>Ile||Segment2>Ile) 
    {
                               ...
                               double KwadratMinOdl) //"maxkatgiecia.nb"
{
 ...
 double StalyZnak=(P1.x*P2.x+P1.y*P2.y)*(odlz2+p1+p2-KwadratMinOdl);
 double ZmiennyZnak=P2.x*P1.y-P1.x*P2.y;
 ZmiennyZnak*=sqrtl(-ZmiennyZnak*(odlz2*odlz2+p1*p1+
...
double CloseRepulsiveKnotEnergy(Knot3D* wskKnot)
{
 ...
 int IleLiczyc=100,
     IleSkakac=wskKnot->IloscSegm/IleLiczyc,
     i=0;
 ...
 Roznica=*wskPomoc1 - wskKnot->wskTablicySegm[nrP(wskKnot->IloscSegm, i+IleSkakac)];
...
double DistantRepulsiveKnotEnergy(Knot3D* wskKnot)
{
 ...
 for(;i<Ilosc;i++)
    {
     Roznica=wskKnot->wskTablicySegm[i]-wskKnot->wskTablicySegm[nrP(i+Ilosc/2,Ilosc)];
     odlkwadrat=Roznica.length()-2*wskKnot->PromienPrzekroju;
